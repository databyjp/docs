...
The vector index type can be set for each collection at creation time, between `hnsw`, `flat`, `dynamic`, and `spfresh` index types.
...
SPFresh is a hybrid disk-based vector index designed for billion-scale datasets with frequent updates. Unlike traditional approaches that require periodic global rebuilds, SPFresh maintains index quality through incremental local rebalancing, making it ideal for collections with high update rates (>1% daily updates).

<Tabs groupId="languages">
  <TabItem value="py" label="Python">
    <FilteredTextBlock
      text={PyCode}
      startMarker="# START ConfigureSPFresh"
      endMarker="# END ConfigureSPFresh"
      language="py"
    />
  </TabItem>
...
### Migration from HNSW to SPFresh

For collections requiring frequent updates without performance degradation, SPFresh provides a better alternative to HNSW:

<Tabs groupId="languages">
  <TabItem value="py" label="Python">
    <FilteredTextBlock
      text={PyCode}
      startMarker="# START MigrateToSPFresh"
      endMarker="# END MigrateToSPFresh"
      language="py"
    />
  </TabItem>
...
<details>
  <summary>When to use SPFresh</summary>
...
:::note SPFresh parameters
SPFresh has its own specific configuration parameters for partition management and rebalancing. See the [configuration reference](/weaviate/config-refs/indexing/vector-index.mdx#spfresh-parameters) for complete details.
:::
