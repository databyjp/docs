...
:::caution Cross-References and Performance
Cross-references can significantly impact query performance, especially for:
- Queries involving multiple levels of cross-references
- Filtered cross-reference searches
- Large-scale data models with complex relationships

Consider these alternatives:
- Embedding related data directly in objects
- Using filters and vector search instead of cross-references
- Denormalizing data for read-heavy workloads

When possible, limit cross-reference depth to a single level and carefully evaluate the performance implications for your specific use case.
:::
...
Collection aliases are alternative names (pointers) for Weaviate collections that allow you to reference a collection by multiple names. When you query using an alias, Weaviate automatically routes the request to the target collection. You can set up collection aliases [programmatically through client libraries](../../manage-collections/collection-aliases.mdx) or by using the <SkipLink href="/weaviate/api/rest#tag/schema">REST endpoints</SkipLink>.
