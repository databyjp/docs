...
- **For most users with HNSW indexes who want the best combination of simplicity, performance, and recall, we recommend 8-bit Rotational Quantization (RQ)**. RQ provides:
  - 4x compression
  - 98-99% recall
  - No configuration or training required
  - Ideal for standard use cases with embeddings from providers like OpenAI
...
- For very large data sets or specialized search needs, consider Product Quantization (PQ), which is highly configurable but requires more expertise to tune.
