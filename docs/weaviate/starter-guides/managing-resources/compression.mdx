...
- PQ compressed vectors typically use 85% to 88% less memory than uncompressed vectors.
...
- BQ compressed vectors use up to 97% less memory than uncompressed vectors.
...
- PQ: Varies based on configuration, typically 90-95% recall
- SQ: 95-97% recall
- RQ: 98-99% recall
- BQ: Varies significantly, typically 80-90% recall depending on data and model characteristics
...
- BQ uses fast, bitwise calculations. BQ's bitwise calculations are extremely efficient. Searches of BQ compressed vectors can be as much as 10 to 20 times as fast as similar searches of uncompressed vectors. BQ is particularly effective for high-dimensionality models like `text-embedding-3` and `Cohere's V3 models`.

- SQ significantly improves search speeds, being 3 to 4 times as fast as searching uncompressed vectors. SQ provides a balanced approach by converting 32-bit floats to 8-bit integers while maintaining good dimensional resolution.
...
- **Binary Quantization (BQ)**: Highest compression (up to 97%), lowest accuracy. Best for high-dimensionality models and scenarios where slight recall reduction is acceptable.
- **Scalar Quantization (SQ)**: Balanced compression and accuracy. Provides a good middle-ground with 75% memory reduction and minimal recall impact.
- **Product Quantization (PQ)**: Most flexible but complex compression method. Suitable for large datasets with advanced requirements.
- **Rotational Quantization (RQ)**: Recommended default for most use cases. Provides 4x compression with 98-99% recall and requires no configuration.

### Specific Recommendations
- For most users with HNSW indexes who want the best combination of simplicity, performance, and recall, **use 8-bit RQ compression**. It provides 4x compression with 98-99% recall and requires no configuration or training.

- For small collections using a flat index, consider BQ index. The BQ index is 32 times smaller and much faster than the uncompressed equivalent.

- For very large datasets or specialized search needs, consider PQ compression. PQ is highly configurable but requires more expertise to tune effectively.

**New in v1.33**: RQ is now the default 8-bit quantization method, offering an excellent balance of performance and recall with minimal configuration.
