...
Weaviate uses multiple types of indexes to facilitate fast, scalable search and filtering. It uses vector indexes (including HNSW, flat, dynamic, and SPFresh) for vector searches and inverted indexes for filtering and keyword searches.
...
- For billion-scale datasets with frequent updates, consider [SPFresh index](#spfresh-indexes) to maintain stable performance.
...
Weaviate offers four types of vector indexes: [Hierarchical Navigable Small World (HNSW) indexes](#hnsw-indexes), [flat indexes](#flat-indexes), [dynamic indexes](#dynamic-indexes), and [SPFresh indexes](#spfresh-indexes).
...
- SPFresh indexes are hybrid disk-based indexes designed for billion-scale datasets with frequent updates, maintaining stable performance during continuous data changes.
...
[SPFresh indexes](/weaviate/concepts/vector-index#spfresh-index) are hybrid disk-based indexes specifically designed for high-update scenarios and billion-scale datasets. SPFresh maintains partition centroids in an in-memory HNSW index for fast candidate selection, while storing vector partitions on disk.

Key characteristics of SPFresh indexes:

- **Incremental updates**: Supports in-place vector updates without requiring global index rebuilds
- **Stable performance**: Maintains consistent search latency and accuracy during continuous data updates
- **Resource efficient**: Uses primarily [*warm* resources](./index.md) for disk storage, with minimal [*hot* resources](./index.md) for the small centroid index in memory
- **Automatic rebalancing**: Uses LIRE (Lightweight Incremental REbalancing) to maintain index quality through local partition management

SPFresh indexes are ideal for:
- Collections with frequent updates (>1% daily update rate)
- Billion-scale datasets where periodic HNSW rebuilds are too expensive
- Applications requiring fresh data without performance degradation
- Use cases needing stable tail latency during updates

Memory requirements for SPFresh are significantly lower than full HNSW indexes, typically requiring only ~10-20GB for billion-scale datasets, as most data resides on disk with only partition centroids kept in memory.
