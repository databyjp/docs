...
Cross-references are best suited for:
- Relationships with infrequent filtering needs
- Low-query-per-second (QPS) environments
- Scenarios with minimal performance requirements

### When to Avoid Cross-References

Avoid cross-references when:
- You need high-performance queries
- Filtering on related objects is crucial
- You have high QPS requirements

### Scalability Considerations

Consider these alternatives:

1. **Embedding Related Data**
   ```json
   {
   ...
   - Cons: Slow writes, limited filter capabilities
...
2. **Denormalized Data Model**
   ...
   ```
   - Pros: Fast reads
   - Cons: Slow writes, cannot filter by author characteristics

:::tip Performance Trade-offs

Choose your data modeling strategy based on your specific performance, query, and write requirements. Each approach has its strengths and limitations.
:::
