...
:::note Resharding Support
Starting in this version, Weaviate supports dynamic resharding through the `/v1/cluster/resharding` API endpoint. This allows you to redistribute data across nodes without downtime, providing greater flexibility in cluster management.
:::
Sharding is configured via the `shardingConfig` object in the collection definition. These parameters are immutable and cannot be changed after the collection is created.
