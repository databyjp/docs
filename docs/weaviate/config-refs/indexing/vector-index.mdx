...
| :------------------ | :----- | :------ | :------------------------------------------------------------------- |
| `vectorIndexType`   | string | `hnsw`  | Optional. The index type - can be `hnsw`, `flat`, `dynamic` or `spfresh`.       |
...
SPFresh is a hybrid disk-based vector index designed for billion-scale datasets that require frequent updates. It combines an in-memory HNSW index for partition centroids with disk-based vector storage, enabling efficient in-place updates without requiring costly global rebuilds.
...
- **Background rebalancing**: Automatically maintains partition quality through local splits and reassignments
...
- **Incremental rebalancing**: Uses LIRE (Lightweight Incremental REbalancing) to maintain index quality
...
| `maxPostingSize`        | integer | 10000   | Yes     | Maximum vectors per partition before split triggers. When a partition exceeds this size, it splits into two partitions with recomputed centroids.                                                                                                                                                                   |
| `minPostingSize`        | integer | 1000    | Yes     | Minimum partition size before merge is considered. Helps prevent overly small partitions that could hurt performance.                                                                                                                                                                                                |
| `reassignRange`         | integer | 64      | Yes     | Number of nearby partitions to check for reassignment during rebalancing. Higher values improve index quality but increase rebalancing overhead.                                                                                                                                                                    |
| `vectorCacheMaxObjects` | integer | `1e12`  | Yes     | Maximum number of objects in the memory cache. By default, this limit is set to one trillion (`1e12`) objects when a new collection is created. For sizing recommendations, see [Vector cache considerations](../../concepts/indexing/vector-index.md#vector-cache-considerations).                            |
| `ef`                    | integer | -1      | Yes     | Search list size for HNSW centroid index. When set to -1, uses dynamic ef adjustment. Higher values improve recall at the cost of slower centroid search. Since centroids are much fewer than vectors, this has less impact on overall performance compared to regular HNSW.                                   |
| `efConstruction`        | integer | 128     | No      | Build parameter for HNSW centroid index. Higher values create better centroid connectivity during initial index construction, improving search quality at the cost of longer build time.                                                                                                                            |
...
### SPFresh configuration tips

To optimize SPFresh performance for your use case, consider these guidelines:

#### For high-update workloads
- **Lower `maxPostingSize` (5000-8000)**: More frequent splits reduce the impact of individual updates but increase rebalancing overhead
- **Moderate `reassignRange` (32-64)**: Balance between index quality and rebalancing cost
- **Monitor partition distribution**: Ensure partitions don't become too imbalanced

#### For billion-scale datasets
- **Higher `maxPostingSize` (10000-15000)**: Reduce the total number of partitions and memory overhead
- **Higher `reassignRange` (64-128)**: Improve index quality for better recall
- **Ensure sufficient IOPS**: Disk performance is critical for partition loading

#### For mixed read/write workloads
- **Balanced `maxPostingSize` (8000-12000)**: Balance between update efficiency and search performance
- **Dynamic `ef` (-1)**: Let Weaviate automatically adjust centroid search parameters
- **Monitor cache hit rates**: Adjust `vectorCacheMaxObjects` based on working set size

:::tip Starting configuration
This set of values provides a good balance for most SPFresh use cases:
...
Starting with Weaviate v1.33, **[8-bit Rotational Quantization (RQ)](../../configuration/compression/rq-compression.md) is enabled by default for all new collections**. This automatic optimization provides up to 4x memory compression and faster query performance while maintaining high recall accuracy. Existing collections (for example restored from backups) are not affected and retain their original configuration.
