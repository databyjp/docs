...
|| `filterStrategy`         | string  | Added in `v1.27.0`. The filter strategy to use for filtering the search results. The filter strategy can be set to `sweeping` or `acorn`. <br/><br/> - `sweeping`: The default filter strategy. <br/> - `acorn`: Uses Weaviate's ACORN implementation for significantly faster filtered vector search, especially for queries with low filter-query vector correlation. Can provide up to 10x performance improvement for filtered queries. [Read more about filtering strategies](../../concepts/filtering.md).|
...
### Enabling ACORN Filter Strategy

To enable the ACORN filter strategy for improved performance in complex filtering scenarios, use the following configuration:
...
> **Note**: ACORN is particularly effective for queries with negatively correlated filters or complex filter combinations that traditionally impact search performance.
