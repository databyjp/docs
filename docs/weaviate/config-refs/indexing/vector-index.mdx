...
| Parameter            | Type    | Default | Details                                                                                                                                                                                   |
| :------------------- | :------ | :------ | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `rq`: `bits`         | integer | `8`     | The number of bits used to quantize each data point. Options are `8` (recommended) or `1`. Different bit depths provide varying compression and performance characteristics.               |
| `rq`: `rescoreLimit` | integer | `-1`    | The minimum number of candidates to fetch before rescoring. For RQ, you can often set this to 0 due to high native recall, maximizing query performance with minimal quality degradation. |
...
Starting with Weaviate v1.33, **[8-bit Rotational Quantization (RQ)](../../configuration/compression/rq-compression.md) is enabled by default for all new collections**. This automatic optimization provides:
- Up to 4x memory compression
- Faster query performance
- High recall accuracy (98-99%)

Existing collections (for example restored from backups) are not affected and retain their original configuration.
