...
SPFresh is a hybrid disk-based vector index optimized for billion-scale datasets with frequent updates. It maintains partition centroids in an in-memory HNSW index while storing vector partitions on disk.
...
| `maxPostingSize` | integer | 10000 | Yes | Maximum vectors per partition before split triggers. When a partition exceeds this size, it automatically splits into two partitions with recomputed centroids. |
| `minPostingSize` | integer | 1000 | Yes | Minimum partition size before merge is considered. Helps prevent partitions from becoming too small after deletions. |
| `reassignRange` | integer | 64 | Yes | Number of nearby partitions to check for reassignment during rebalancing. Higher values improve quality but increase overhead. |
| `vectorCacheMaxObjects` | integer | `1e12` | Yes | Maximum number of objects in the memory cache. By default, this limit is set to one trillion (`1e12`) objects when a new collection is created. For sizing recommendations, see [Vector cache considerations](../../concepts/indexing/vector-index.md#vector-cache-considerations). |
| `ef` | integer | -1 | Yes | Search list size for HNSW centroid index. Uses dynamic ef by default (-1). Higher values improve accuracy but slow down centroid search. |
| `efConstruction` | integer | 128 | No | Build parameter for HNSW centroid index. Higher values create better quality centroid index but increase build time. |
