...
### Memory Considerations in Scaling

When scaling your Weaviate deployment, memory management becomes crucial. The HNSW index must fit entirely in memory. A rough estimate for memory usage is:
$2 * numDimensions * numVectors * 4B$

For example, 100M vectors with 512 dimensions would require approximately 400GiB of memory. It's recommended to keep 20% extra memory for page cache and additional requirements.
...
When implementing sharding, consider these techniques to manage memory usage:
- Use vector compression techniques
- Reduce vector dimensionality using Principal Component Analysis (PCA)
- Adjust `maxConnections` and increase `ef` and `efConstruction` parameters
...
Utilize the `ASYNC_REPLICATION` environment variables to optimize replication performance. This can help improve write throughput and reduce latency in distributed deployments.
