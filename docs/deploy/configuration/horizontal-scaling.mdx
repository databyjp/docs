...
Here's a comprehensive example showcasing how to combine sharding and replication for a scalable, highly available collection:

<Tabs groupId="languages">
  <TabItem value="py" label="Python">
```python
client.collections.create(
    name="ScalableCollection",
    vectorizer_config=vectorizer,
    replication_config=Configure.replication(
        factor=3,              # Three copies of data
        async_enabled=True,    # Background replication
        deletion_strategy=ReplicationDeletionStrategy.TIME_BASED_RESOLUTION
    ),
    sharding_config=Configure.sharding(
        desired_count=4,       # 4 physical shards
        virtual_per_physical=128  # Default virtual shard configuration
    )
)
```
  </TabItem>
...
client.collections.create({
    ...
    replicationConfig: {
        factor: 3,
        asyncEnabled: true,
        deletionStrategy: ReplicationDeletionStrategy.TIME_BASED_RESOLUTION
    },
    shardingConfig: {
        desiredCount: 4,
...
When combining sharding and replication, consider these key factors:

1. **Performance Improvements**
   - Sharding distributes data across multiple nodes
   - Replication provides read scalability
   - Combined approach enables parallel processing and redundancy

2. **Fault Tolerance**
   - Multiple replicas protect against node failures
   - Sharding prevents single point of data loss
   - Recommended minimum: 3-node cluster with replication factor of 3
...
- Monitor performance and adjust shard/replica count
- Use async replication for write-heavy workloads
